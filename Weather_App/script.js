...
const API_KEY = '05d507cfddb24fcf8cd173248242709';
...
const displayHourlyForecast = (hourlyData) => {
    ...
    // Filter the hourly data to only include the net 24 hours
    ...
    hourlyWeatherDiv.innerHTML = next24HoursData.map(item => {
        ...
        return ` <li class="weather-item">
                        <p class="time">${time}</p>
                        <img src="icons/${weatherIcon}.svg" class="weather-icons">
...
const getWeatherDetails = async (API_URL) => {
   ...
   try {
        // Fetch weather data from the API and prase the response as JSON
        ...
        currentWeatherdiv.querySelector(".weather-icons").src = `icons/${weatherIcon}.svg`;
        currentWeatherdiv.querySelector(".temperature").innerHTML = `${temperature}<span>&#176;C</span>`;
        currentWeatherdiv.querySelector(".description").innerText = description;

        // combined hourly data from today and ommorrow
...
// Initial weather request for Delhi as the default city
setupWeatherRequest("delhi");