...
let overScores = [];
let currentTeam = 1;
let teamScores = [[], []]; // Stores Scores for both teams
    ...
    if (wickets < 10 && ballCount < 6) {
        ...
        if (type !== "dot") {
            if (type !== "wicket") {
                ...
                currentScore += runs; // Update current score only for runs
            } else {
                wickets++;
                if (wickets === 11) {
                    endOver();
...
function addExtraBall(type) {
    if (wickets < 10 && ballCount < 6) {
        ...
        if (type === "wide" || type === "no") {
            ...
            currentOverScore += 1;
            currentScore += 1; // Update current score only for extras
        }
        ballsContainer.appendChild(ball);
...
function endOver() {
    // Push the current over score to the array of over scores and team scores
    overScores.push(currentOverScore);
    teamScores[currentTeam - 1].push(currentOverScore);
    // Add the current over score to the total score
    totalScore += currentOverScore;

    // Update local storage with the updated team scores
    updateLocalStorage();

    // Get the cumulative score display container
    const cumulativeScoreDisplay = document.getElementById("cumulative-score");

    // Check if the over score element already exists
    let overScoreElement = document.getElementById("over-score");
    if (!overScoreElement) {
        // Create a new element if it doesn't exist
        overScoreElement = document.createElement("div");
        overScoreElement.id = "over-score";
        cumulativeScoreDisplay.appendChild(overScoreElement);
    }

    // Update the over score element text content
    overScoreElement.innerText = `Team ${currentTeam}, Over ${currentOver} : ${currentOverScore} runs`;

    // Increment the current over number
    currentOver++;

    // Reset ball count and current over score
    ballCount = 0;
    currentOverScore = 0;

    // Update the total score and wickets display
    const scoreDisplay = document.getElementById("score-display");
    scoreDisplay.innerText = `Team ${currentTeam} Total Score: ${totalScore} runs, Wickets: ${wickets}`;

    // Check if the innings has ended or all wickets have fallen
    if (currentOver > 20 || wickets === 10) {
        // End the innings if true
        endInnings();
    } else {
        // Otherwise, disable buttons and show new over button
        disableButtons();
        showNewOverButton();
    }
...
function showNewOverButton() {
    ...
    document.querySelector(".button-container").appendChild(newOverButton);
}
...
function resetOver() {
    ...
    document
        ...
        .removeChild(
            document.querySelector(".button-container button:last-child")
        );
    const scoreDisplay = document.getElementById("score-display");
...
function updateLocalStorage() {
    localStorage.setItem("teamScore", JSON.stringify(teamScores));
}

// Ensure all functions are defined before adding event listeners
document.addEventListener("DOMContentLoaded", () => {
    // Load scores from local storage if available
    const savedScores = localStorage.getItem("teamScore");
    if (savedScores) {
        teamScores = JSON.parse(savedScores);
        // You might want to set currentTeam and other variables here as well based on stored data
    }
